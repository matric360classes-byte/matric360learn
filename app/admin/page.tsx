...
export default function AdminPage(){
  ...
  const Item=({label,path}:{label:string,path:string})=>(<div onClick={()=>{setMenuOpen(false); router.push(path)}} style={{padding:"10px 0 10px 16px", color:"#9ca3af"}}>{label}</div>);
    ...
    <div style={{minHeight:"100vh", background:"#0f0f14", color:"white", fontFamily:"system-ui", paddingBottom:80}}>
      ...
      {menuOpen && (
        <div style={{margin:"0 12px", background:"#181821", borderRadius:20, padding:12, border:"1px solid #222"}}>
          ...
          <div onClick={()=>{setMenuOpen(false); router.push("/admin/beta")}} style={{padding:10}}>🐞 Beta & QA</div>
          <div onClick={()=>{setMenuOpen(false); router.push("/admin/users")}} style={{padding:10, fontWeight:"bold", color:"white", background:"#222", borderRadius:10}}>👥 Users ← NOW WIRED</div>
        </div>
      )}
