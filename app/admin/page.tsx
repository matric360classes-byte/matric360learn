...
export default function AdminPage(){
  ...
  const [exp,setExp]=useState({content:true,kb:false,videos:false,exam:false,gen:true,beta:false});
  ...
  useEffect(()=>{
    async function load(){
      ...
      const missingNodes = data.filter((t:any)=>!t.nodes || t.nodes.length<5).length;
  ...
  return(
    <div style={{background:"#0b0c14",minHeight:"100vh",color:"#fff",paddingBottom:90}}>
      {/* GREEN BANNER WITH HAMBURGER - YOUR CIRCLED BUTTON */}
      <div style={{margin:12,background:"#121a14",border:"1px solid #1e3a2a",borderRadius:20,padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        ...
        <button onClick={()=>setOpen(!open)} style={{minWidth:40,minHeight:40,borderRadius:12,background:"#1a1c2e",border:"1px solid #252a44",color:"#fff",fontSize:18}}>
          {open ? "✕" : "☰"}
        </button>
      </div>

      {/* MENU CARD - OPENS WHEN YOU CLICK THAT HAMBURGER - EXACTLY YOUR PHOTO 1 & 2 */}
      {open && (
      ...
      <div style={{padding:"0 16px",display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:8}}>
        ...
        <Link href="/admin/studio" style={{background:"#8b8bff",color:"#fff",padding:"12px 16px",borderRadius:16,textDecoration:"none",fontWeight:800,fontSize:13}}>Open Content<br/>Studio</Link>
      </div>
      ...
      <div style={{padding:"0 16px"}}>
        ...
        <div style={{background:"#151725",border:"1px solid #1e2238",borderRadius:20,padding:12,fontSize:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1.2fr 0.6fr 1fr 0.5fr 0.6fr",padding:"10px 8px",borderTop:"1px solid #1e2238",fontWeight:700}}><span>Physical Sciences</span><span style={{textAlign:"center"}}>{stats.physics}</span><span style={{textAlign:"center"}}>142</span><span style={{textAlign:"center"}}>89</span><span style={{textAlign:"center",color:"#facc15"}}>0</span></div>
        </div>
      </div>
    </div>
  )
