...
export default function NewVideoPage(){
  ...
  const save=async()=>{
    ...
    try{
      ...
      const kbPick=kb.find(k=>k.caps_code===form.topic);
      ...
      alert("✅ Saved! Now shows under Nodes A-E in HD");
    ...
    <div style={{background:"#0b0c14",minHeight:"100vh",color:"#fff",paddingBottom:90}}>
      ...
      <div style={{margin:"12px",background:"#1c1e2e",borderRadius:24,padding:16,border:"1px solid #252a44"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:12}}>
          <select value={form.subject} onChange={e=>setForm({...form,subject:e.target.value,unit:"",topic:""})} style={{background:"#12131f",border:"1px solid #252a44",borderRadius:14,padding:"14px",color:"#fff"}}><option value="">Subject...</option>{subjects.map(s=><option key={s} value={s}>{s}</option>)}</select>
          <select value={form.unit} onChange={e=>setForm({...form,unit:e.target.value,topic:""})} style={{background:"#12131f",border:"1px solid #252a44",borderRadius:14,padding:"14px",color:"#fff"}}><option value="">Unit...</option>{units.filter(Boolean).map(u=><option key={u} value={u}>{u}</option>)}</select>
          <select value={form.topic} onChange={e=>{const t=e.target.value; const pick=kb.find(k=>k.caps_code===t); setForm({...form,topic:t,title: pick?.topic || form.title})}} style={{background:"#12131f",border:"1px solid #252a44",borderRadius:14,padding:"14px",color:"#fff"}}><option value="">Topic...</option>{topics.map(k=><option key={k.id} value={k.caps_code}>{k.caps_code} - {k.topic}</option>)}</select>
        </div>
