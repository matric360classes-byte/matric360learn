...
export default function NewVideoPage(){
  ...
  const save=async()=>{
    ...
    const {data,error}=await supabase.from("videos").insert([{
      youtube_url:form.url, youtube_id:yid, provider:form.provider.toLowerCase(),
      duration_sec: form.duration? parseInt(form.duration): null,
      subject:form.subject, unit:form.unit, topic: kbPick?.topic || form.topic, caps_code: form.topic, caps_topic_id: kbPick?.id,
      title: form.title || kbPick?.topic || yid, description: form.description,
      thumbnail_url: form.thumb || `https://img.youtube.com/vi/${yid}/hqdefault.jpg`,
      order_num: parseInt(form.order||"0"), status: form.status.toLowerCase()
    }]).select();
    setSaving(false);
    if(error) alert("Error: "+error.message); else { alert("Video saved!"); router.push("/admin/videos"); }
  };
