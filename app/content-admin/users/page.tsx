...
export default function ContentAdminUsers(){
  ...
  useEffect(()=>{
    // TODO: Replace with Supabase: supabase.from('profiles').select('*')
    // Mock for now - will be replaced by real data
    const mock:User[] = [
      {
        id:"1",
  ...
  return(
    <div style={{background:"#0a0d1a",minHeight:"100vh",color:"white",padding:"16px"}}>
      ...
      <div style={{display:"flex",gap:10,marginTop:16}}>
        ...
        <select style={{background:"#1a1d2f",border:"1px solid #252a44",borderRadius:12,padding:"12px",color:"white"}}>
          ...
          <div key={user.id} style={{background:"#1a1d2f",border:"1px solid #252a44",borderRadius:16,padding:14,marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              ...
              <div style={{display:"flex",gap:6}}>
                ...
                <span style={{fontSize:10,background:user.subscription_status==="active"?"#10b98120":"#2a2d4a",color:user.subscription_status==="active"?"#10b981":"#94a3b8",padding:"4px 8px",borderRadius:999}}>{user.subscription_status}</span>
              </div>
            </div>
            ...
            {/* SUBSCRIPTION - THIS IS NEW VS REFERENCE APP */}
