...
export default function Factory(){
  ...
  const processAll=async()=>{
    ...
    while(true){
      ...
      total+=j.processed||0; setLogs(l=>l+`\n+${j.processed} processed | remaining ${j.remaining} | total ${total}`); await load();
  ...
  return (
    ...
    <button disabled={busy} onClick={processAll} className="bg-black text-white px-6 py-3 rounded">{busy?"Processing...":`Process All Queued (${stats.queued})`}</button>
