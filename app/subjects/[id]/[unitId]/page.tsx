  ...
  if(subj){
    for(const s of subj.sections){
      for(const u of s.units){
        if(u.id===params.unitId){ foundUnit=u; break; }
      }
    }
  }
  if(!foundUnit) return <div style={{background:"#0f0f13",minHeight:"100vh",color:"white",padding:20}}>Not found</div>;
  return(
    <div style={{background:"#0f0f13",minHeight:"100vh",color:"white",padding:20}}>
      ...
      <h1 style={{fontSize:28,fontWeight:800,marginTop:16}}>{foundUnit.title}</h1>
      <div style={{marginTop:20,display:"flex",flexDirection:"column",gap:12}}>
        {(foundUnit.topics||[]).map((t:any)=>(
